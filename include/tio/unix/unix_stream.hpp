...
class unix_stream {
public:
  [[nodiscard]] static auto connect(const detail::unix_addr& addr) -> result<unix_stream>;

  [[nodiscard]] static auto pair() -> result<std::pair<unix_stream, unix_stream>>;

  [[nodiscard]] static auto from_raw_fd(int fd) noexcept -> unix_stream {
    return unix_stream{detail::fd_guard{fd}};
  }

  explicit unix_stream(detail::fd_guard fd) noexcept : fd_{std::move(fd)} {}

  unix_stream(unix_stream&&) noexcept = default;
  auto operator=(unix_stream&&) noexcept -> unix_stream& = default;

  unix_stream(const unix_stream&) = delete;
  auto operator=(const unix_stream&) -> unix_stream& = delete;
...
static_assert(source<unix_stream>);
